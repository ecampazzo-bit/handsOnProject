...
SELECT id, nombre, categoria_id FROM public.servicios ORDER BY nombre;

-- 2. Ver las categorías
SELECT id, nombre FROM public.categorias ORDER BY nombre;

-- 3. Ver prestadores creados
SELECT id, usuario_id, descripcion_profesional, created_at FROM public.prestadores ORDER BY created_at DESC LIMIT 10;

-- 4. Ver usuarios y sus tipos
SELECT id, email, nombre, apellido, tipo_usuario, verificado FROM public.users ORDER BY created_at DESC LIMIT 10;
...
SELECT id, nombre, categoria_id FROM public.servicios WHERE nombre ILIKE '%Alb%';
...
DO $$ DECLARE
  v_construccion_id bigint;
BEGIN
  -- Obtener ID de categoría "Construcción y Albañilería"
  SELECT id INTO v_construccion_id FROM public.categorias WHERE nombre = 'Construcción y Albañilería' LIMIT 1;
  
  -- Si no existe, crearla
  IF v_construccion_id IS NULL THEN
    INSERT INTO public.categorias (nombre) VALUES ('Construcción y Albañilería') RETURNING id INTO v_construccion_id;
  END IF;
  
  -- Insertar servicios de construcción si no existen
  INSERT INTO public.servicios (nombre, categoria_id) VALUES
    ('Albañil', v_construccion_id),
    ('Constructor', v_construccion_id),
    ('Maestro mayor de obras', v_construccion_id)
