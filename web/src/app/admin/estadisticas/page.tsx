...
export default function EstadisticasDashboard() {
  ...
  useEffect(() => {
    ...
    if (filtroCategoria !== 'all' || busquedaPrestador.trim() !== '' || (filtroLatitud !== null && filtroLongitud !== null)) {
      loadPrestadores()
    } else {
      setPrestadores([])
    }
  }, [filtroCategoria, busquedaPrestador, filtroLatitud, filtroLongitud, filtroRadio])
        ...
        if (prestadoresPorCategoriaData) {
          ...
          if (filtroCategoria !== 'all') {
            prestadoresFiltrados = prestadoresPorCategoriaData.filter(
              (item: { categoria_id: number; categoria_nombre: string; cantidad_prestadores: number }) => item.categoria_id === filtroCategoria
            )
          }
    ...
    try {
      ...
      cotizacionesData?.forEach((cot) => {
        if (cot.estado in cotizacionesPorEstado) {
          cotizacionesPorEstado[cot.estado as keyof typeof cotizacionesPorEstado]++
        }
      })
      ...
      if (prestadoresPorCategoriaData) {
        ...
        const categoriaMap = new Map<number, { nombre: string; count: number }>()
        ...
        prestadoresPorCategoria = Array.from(categoriaMap.entries()).map(([id, data]) => ({
          categoria_id: id,
          categoria_nombre: data.nombre,
          cantidad_prestadores: prestadoresUnicosPorCategoria.get(id)?.size || 0,
        }))
      }
  ...
  const loadPrestadores = async () => {
    try {
      ...
      if (filtroCategoria !== 'all') {
        console.log('Filtrando por categoría:', filtroCategoria)
        console.log('Prestadores antes del filtro:', prestadoresConCategoria.length)
        prestadoresFiltrados = prestadoresFiltrados.filter((p: any) => {
          const tieneCategoria = p.todas_las_categorias_ids && 
            p.todas_las_categorias_ids.some((catId: number) => Number(catId) === Number(filtroCategoria))
          if (!tieneCategoria) {
            console.log(`Prestador ${p.nombre} ${p.apellido} no tiene categoría ${filtroCategoria}. Categorías:`, p.todas_las_categorias_ids)
          }
          return tieneCategoria
        })
        console.log('Prestadores después del filtro:', prestadoresFiltrados.length)
      }
      ...
      if (filtroLatitud !== null && filtroLongitud !== null) {
        ...
        prestadoresFiltrados = prestadoresFiltrados.filter((p) => {
          const prestadorConUbicacion = prestadoresConUbicacion.find((pu) => pu?.id === p.id)
          return prestadorConUbicacion !== null && prestadorConUbicacion !== undefined
        })
      }
  ...
  const trabajosEnCurso = (estadisticas?.trabajosPorEstado.programado || 0) +
      ...
      <main className="container mx-auto px-4 py-8">
        {loading ? (
          ...
          <>
            ...
            <div className="bg-white rounded-lg shadow p-6 mb-6">
              <h2 className="text-xl font-semibold mb-4 text-gray-900">Presupuestos (Cotizaciones)</h2>
              <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                <div className="bg-blue-50 rounded-lg p-4">
                  <p className="text-sm text-gray-600 mb-1">Total</p>
                  <p className="text-3xl font-bold text-blue-600">{estadisticas.totalCotizaciones}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="text-sm text-gray-600 mb-1">Enviadas</p>
                  <p className="text-3xl font-bold text-gray-700">{estadisticas.cotizacionesPorEstado.enviada}</p>
                </div>
                <div className="bg-yellow-50 rounded-lg p-4">
                  <p className="text-sm text-gray-600 mb-1">Vistas</p>
                  <p className="text-3xl font-bold text-yellow-600">{estadisticas.cotizacionesPorEstado.vista}</p>
                </div>
                <div className="bg-green-50 rounded-lg p-4">
                  <p className="text-sm text-gray-600 mb-1">Aceptadas</p>
                  <p className="text-3xl font-bold text-green-600">{estadisticas.cotizacionesPorEstado.aceptada}</p>
                </div>
                <div className="bg-red-50 rounded-lg p-4">
                  <p className="text-sm text-gray-600 mb-1">Rechazadas</p>
                  <p className="text-3xl font-bold text-red-600">{estadisticas.cotizacionesPorEstado.rechazada}</p>
                </div>
                <div className="bg-orange-50 rounded-lg p-4">
                  <p className="text-sm text-gray-600 mb-1">Expiradas</p>
                  <p className="text-3xl font-bold text-orange-600">{estadisticas.cotizacionesPorEstado.expirada}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6 mb-6">
              <h2 className="text-xl font-semibold mb-4 text-gray-900">Trabajos</h2>
              <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
                <div className="bg-blue-50 rounded-lg p-4">
                  <p className="text-sm text-gray-600 mb-1">Total</p>
                  <p className="text-3xl font-bold text-blue-600">{estadisticas.totalTrabajos}</p>
                </div>
                <div className="bg-green-50 rounded-lg p-4">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-3xl font-bold text-gray-700">{estadisticas.trabajosPorEstado.programado}</p>
                </div>
                <div className="bg-indigo-50 rounded-lg p-4">
                  <p className="text-sm text-gray-600 mb-1">En Progreso</p>
                  <p className="text-3xl font-bold text-indigo-600">{estadisticas.trabajosPorEstado.en_progreso}</p>
                </div>
              </div>
            </div>
            ...
            {(filtroCategoria !== 'all' || busquedaPrestador.trim() !== '' || (filtroLatitud !== null && filtroLongitud !== null)) && (
              <div className="bg-white rounded-lg shadow p-6">
                <h2 className="text-xl font-semibold mb-4 text-gray-900">
                  Prestadores Filtrados
                  {prestadores.length > 0 && (
                    <span className="text-lg font-normal text-gray-600 ml-2">
                      ({prestadores.length} {prestadores.length === 1 ? 'prestador' : 'prestadores'})
                    </span>
                  )}
                </h2>
                {loading ? (
                  ...
                  <p className="text-gray-500 text-center py-8">No se encontraron prestadores con los filtros seleccionados</p>
                ) : (
